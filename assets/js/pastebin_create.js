...
const RATE_LIMIT_KEY    = "pastebin_rate";
...
// Pastebin sanitizes script injections but keeps < > for code display
function sanitizeInput(str) {
  return str.replace(/javascript:/gi, "").replace(/on\w+\s*=/gi, "");
}
...
function getRateRecord(){try{const r=localStorage.getItem(RATE_LIMIT_KEY);if(r){const p=JSON.parse(r);if(!p.timestamps)p.timestamps=[];return p;}}catch{}return{timestamps:[]};}
function saveRateRecord(r){try{localStorage.setItem(RATE_LIMIT_KEY,JSON.stringify(r));}catch{}}
function checkFrontendLimit(){let r=getRateRecord();const now=Date.now();r.timestamps=r.timestamps.filter(t=>now-t<RATE_LIMIT_WINDOW);saveRateRecord(r);if(r.timestamps.length<RATE_LIMIT_MAX)return{allowed:true};return{allowed:false,resetsAt:new Date(r.timestamps[0]+RATE_LIMIT_WINDOW)};}
function incrementFrontendCount(){const r=getRateRecord();const now=Date.now();r.timestamps=(r.timestamps||[]).filter(t=>now-t<RATE_LIMIT_WINDOW);r.timestamps.push(now);saveRateRecord(r);}
async function checkDatabaseLimit(){
  ...
  const{count,error}=await supabase.from("notes").select("id",{count:"exact",head:true}).eq("created_by_ip",ip).eq("type","paste").gte("created_at",since);
  if(error)return{allowed:true,ip};
  return{allowed:count<RATE_LIMIT_MAX,ip};
}
function formatTimeLeft(r){const s=Math.ceil((r.getTime()-Date.now())/1000);return s<=60?`${s} second${s===1?"":"s"}`:`${Math.ceil(s/60)} minutes`;}
...
createBtn.addEventListener("click", async () => {
  ...
  if (!message) { showError("Please paste some code or text before creating."); return; }
  ...
  try {
    ...
    const { data, error } = await supabase.from("notes").insert([{
      ...
      has_password: password.length > 0, created_by_ip: db.ip, type: "paste",
    }]).select();
    if (error) throw error;

    incrementFrontendCount();
    const base = `${window.location.origin}/read-pastebin.html?id=${data[0].id}`;
    showSuccess(randomKey ? `${base}#${randomKey}` : base);

  } catch (err) {
    console.error("Pastebin create error:", err);
...
function setLoading(l){createBtn.disabled=l;createBtnText.textContent=l?"Creating…":"Create Private Paste";createSpinner.classList.toggle("hidden",!l);}
