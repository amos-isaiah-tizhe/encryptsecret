...
copyMsgBtn?.addEventListener("click", async () => {
  try { await navigator.clipboard.writeText(messageBoxEl.textContent); copyMsgText.textContent="Copied!"; setTimeout(()=>{copyMsgText.textContent="Copy Password";},2000); }
  catch { window.prompt("Copy this password:", messageBoxEl.textContent); }
});

window.addEventListener("DOMContentLoaded", async () => {
  if (!noteId) { showState(stateError); errorMsg.textContent="No password ID in this link."; return; }
  showState(stateLoading);
  try {
    const { data, error } = await supabase.from("notes")
      .select("id,message,has_password,expires_at,type").eq("id",noteId).eq("type","password").single();
    ...
    if (noteHasPassword) {
      ...
      lockedDesc.textContent = "This password is protected. Enter the link password to reveal it.";
  ...
  } catch (err) { console.error("EncryptPass load error:", err); showState(stateError); }
});
...
unlockBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!decrypted) { wrongPasswordAlert.classList.remove("hidden"); unlockBtn.disabled=false; unlockBtnText.textContent="Reveal Password"; unlockSpinner.classList.add("hidden"); return; }
    messageBoxEl.textContent = decrypted;
    showState(stateRevealed);
    await supabase.from("notes").delete().eq("id", noteId);
  } catch (err) {
    wrongPasswordAlert.classList.remove("hidden");
    unlockBtn.disabled=false; unlockBtnText.textContent="Reveal Password"; unlockSpinner.classList.add("hidden");
  }
});
